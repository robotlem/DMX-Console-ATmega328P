...
// The comments in this program are only available in German
...
/* ---  DMX-Controller V2 ---
  ...
  Autor:                S. Pauthner
  Änderungsdatum:       8.12.2021 - 12.12.2021
...
void setup(){
  ...
  for (size_t i = 0; i < 4; i++) {    // Anfangsmodus laden
...
void loop(){
  ...
  if (printRefresh == true && millis()-time > 100) {   // LCD updaten
    lcd.clear();
